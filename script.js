(function () {
  ...
  /* Formulario de contacto
     Envía los datos por AJAX (fetch) al endpoint configurado en el
     atributo action= del <form> (ver comentario en index.html para
     la configuración de ese endpoint). No usa mailto: y no expone
     ninguna credencial: el envío real del email lo procesa el
     servicio externo (Formspree) del lado del servidor. */
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function (event) {
      ...
      /* Honeypot: si este campo oculto viene completo, es un bot.
         Se ignora la petición silenciosamente. */
      ...
      var originalLabel = contactSubmitBtn ? contactSubmitBtn.textContent : '';
      contactSending = true;
      if (contactSubmitBtn) {
        contactSubmitBtn.disabled = true;
        contactSubmitBtn.textContent = t('contacto.enviando') || 'Enviando...';
      }
      showStatus('');
      ...
      fetch(endpoint, {
        ...
        showStatus(t('contacto.gracias') || 'Gracias. Tu mensaje fue enviado correctamente. Te responderemos a la brevedad.');
        contactForm.reset();
      }).catch(function () {
        showStatus(t('contacto.error') || 'No pudimos enviar tu mensaje. Por favor, intentá nuevamente o escribinos a matiasparisimastering@gmail.com.');
      }).finally(function () {
